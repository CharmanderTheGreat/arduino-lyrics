# Aphrodite by Ridleys Lyrics using Arduino Uno R3 with I2C Screen Display
...
The `aphrodite-lyrics.lrc` file below contains the full lyrics with line-level timestamps, in the standard LRC format. It's included so the timing data can be reused or ported to other languages/platforms if you don't want to use the Arduino sketch as-is.
...
1. Open `aphrodite-lyrics.ino` from this repo in Arduino IDE.
...
| File | Description |
|------|--------------|
| `aphrodite-lyrics.ino` | Arduino sketch with the word-timestamp array and LCD display logic |
| `aphrodite-lyrics.lrc` | Lyrics with timestamps in standard LRC format |
