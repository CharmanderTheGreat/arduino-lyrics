# Isip by Healy After Dark Lyrics using Arduino Uno R3 with I2C Screen Display

A timestamp-synced lyric display for the song "**Isip**", built on an Arduino Uno R3 and a 16x2 I2C LCD screen. Words appear on the screen in sync with playback time, scrolling across two rows as the song plays.
...
The `isip-lyrics.lrc` file below contains the full lyrics with line-level timestamps, in the standard LRC format. It's included so the timing data can be reused or ported to other languages/platforms if you don't want to use the Arduino sketch as-is.
...
1. Open `isip-lyrics.ino` from this repo in Arduino IDE.
...
| File | Description |
|------|--------------|
| `isip-lyrics.ino` | Arduino sketch with the word-timestamp array and LCD display logic |
| `isip-lyrics.lrc` | Lyrics with timestamps in standard LRC format |
